...
package com.ngapp.quottie.home.fake
...
/** Minimal in-memory fake covering exactly what [com.ngapp.quottie.home.HomeViewModel] uses. */
class FakeQuoteRepository : QuoteRepository {

    var quoteOfTheDayResult: Result<QuoteResource, DataError.Network> =
        Result.Error(DataError.Network.UNKNOWN, Exception("not configured"))
    var randomQuotesResult: Result<List<QuoteResource>, DataError.Network> = Result.Success(emptyList())
    ...
    override suspend fun getQuoteOfTheDay(): Flow<Result<QuoteResource, DataError.Network>> =
        flowOf(quoteOfTheDayResult)

    override suspend fun saveQuoteBookmark(quote: QuoteResource, isBookmarked: Boolean) {
        bookmarks.value = bookmarks.value + (quote.id to quote.copy(isBookmarked = isBookmarked).let {
            QuoteResourceEntity(it.id, it.content, it.author, it.length, it.tags, it.isBookmarked)
        })
    }
